// Vault50Bot — webhook + deposit watchers (Render-ready)
...
const ENABLE_WATCH  = String(process.env.ENABLE_WATCHERS || 'false').toLowerCase() === 'true';
...
app.listen(PORT, async () => {
  ...
  try {
    if (PUBLIC_URL) {
      ...
      console.warn('⚠️ PUBLIC_URL not set; set webhook manually via Telegram API.');
    }
  } catch (e) { console.error('Failed to set webhook:', e?.message || e); }
});
...
/* ---------- COMMANDS ---------- */
bot.onText(/^\/start(?:@\w+)?$/i, (msg) => bot.sendMessage(msg.chat.id, '🤖 Vault50Bot online. Use /help for commands.'));
bot.onText(/^\/help(?:@\w+)?$/i, (msg) => bot.sendMessage(msg.chat.id,
`🧭 Vault50Bot Commands
/pool — show totals
/proof — payout proof
/stats — system stats
...
bot.onText(/^\/stats(?:@\w+)?$/i, (msg) => {
  const up = Math.floor(process.uptime()); const h = Math.floor(up/3600); const m = Math.floor((up%3600)/60);
  bot.sendMessage(msg.chat.id, `📈 Stats\nUptime: ${h}h ${m}m\nWatchers: ${ENABLE_WATCH ? 'ON' : 'OFF'}`);
});
bot.onText(/^\/announce (.+)/i, async (msg, m) => {
...
if (ENABLE_WATCH) {
  ...
  // BTC watcher via mempool.space
  const pollBTC = async () => {
    if (!ADDR.BTC) return;
    try {
      const [txs, mem, info] = await Promise.all([
      ...
      const items = [...(txs.data||[]), ...(mem.data||[])];
      const funded = info.data?.chain_stats?.funded_txo_sum || 0;
      const spent  = info.data?.chain_stats?.spent_txo_sum  || 0;
      const totalBtc = (funded - spent) / 1e8;

      for (const tx of items.slice(0, 40)) {
        const id = tx.txid;
        if (!id || tx._v50seen) continue;
  ...
  // EVM watcher (ETH/BNB)
  const makeEvmWatcher = (label, provider, toAddr) => {
    let last = 0;
    return async () => {
      if (!toAddr) return;
      try {
  ...
  const pollETH = makeEvmWatcher('ETH', providerETH, ADDR.ETH);
  const pollBNB = makeEvmWatcher('BNB', providerBNB, ADDR.BNB);
  ...
  const pollSOL = async () => {
    ...
    try {
      ...
      for (const s of list) {
        ...
        if (!sig || s._v50seen) continue;
        const tx = await solConn.getParsedTransaction(sig, { maxSupportedTransactionVersion: 0 });
